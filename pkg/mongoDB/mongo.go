package mongoDB

import (
	"context"
	"go.mongodb.org/mongo-driver/mongo"
	"go.mongodb.org/mongo-driver/mongo/options"
	"go.mongodb.org/mongo-driver/mongo/readpref"
	"superTools-background/pkg/setting"
	"time"
)

/**
* @Author: super
* @Date: 2021-02-02 11:46
* @Description:
**/

func NewMongoDBEngine(mongoDbSetting *setting.MongoDBSettingS) (*mongo.Client, error) {
	var client *mongo.Client
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(mongoDbSetting.Timeout))
	opt := options.Client().ApplyURI(mongoDbSetting.Url)
	opt.SetMaxPoolSize(mongoDbSetting.MaxPoolSize)
	if client, err := mongo.Connect(ctx, opt); err != nil {
		return nil, err
	} else {
		ctx2, _ := context.WithTimeout(context.Background(), time.Duration(mongoDbSetting.Timeout))
		err := client.Ping(ctx2, readpref.Primary())
		if err != nil {
			return nil, err
		}
	}
	return client, nil
}
